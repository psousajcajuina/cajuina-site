...
function MapController({
  defaultPosition,
  distribuidores,
  sortedDistribuidores,
  setSortedDistribuidores,
  setSelectedMarker,
}: {
  defaultPosition: google.maps.LatLngLiteral;
  distribuidores: Distribuidor[];
  sortedDistribuidores: Distribuidor[];
  ...
  const handlePlaceSelect = async (place: google.maps.places.Place | null) => {
    ...
    if (nearby.length > 0) {
      ...
      nearby.slice(0, 3).forEach((d) => bounds.extend({ lat: d.lat, lng: d.lng }));
  ...
  return <AutocompleteWebComponent onPlaceSelect={handlePlaceSelect} onReset={handleReset} />;
}

function Gmaps({ distribuidores, defaultPosition }: Props) {
  const [sortedDistribuidores, setSortedDistribuidores] = useState(distribuidores);
          ...
          <div className="flex w-full flex-col gap-4">
            ...
            <div className="flex justify-start p-0 lg:hidden">
              <MapController
                ...
                sortedDistribuidores={sortedDistribuidores}
            ...
            <div className="[&_button]:font-inter! mt-auto flex gap-3 [&_button]:h-[45px] [&_button]:text-xs [&_button]:font-medium [&_button]:lg:h-[65px] [&_button]:lg:text-xl">
              ...
              <button className="btn-yellow flex-1">SEJA UM DISTRIBUIDOR</button>
            </div>
          </div>
        </div>
      </div>
